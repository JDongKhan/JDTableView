...
@implementation JDBlockDescription

- (id)initWithBlock:(id)block {
    if (self = [super init]) {
        _block = block;
        
        struct JDBlock *blockRef = (__bridge struct JDBlock *)block;
        _flags = blockRef->flags;
        _size = blockRef->descriptor->size;
        
        if (_flags & JDBlockFlagsHasSignature) {
            void *signatureLocation = blockRef->descriptor;
            signatureLocation += sizeof(unsigned long int);
            signatureLocation += sizeof(unsigned long int);
            
            if (_flags & JDBlockFlagsHasCopyDispose) {
